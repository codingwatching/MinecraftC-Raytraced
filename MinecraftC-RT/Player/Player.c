...
bool PlayerIsPushable(Player player)
{
	return !player->Removed;
}
...
void PlayerKnockback(Player player, Entity entity, int damage, int x, int z)
{
	float len = length2f((float2){ x, z });
	player->Delta /= 2.0;
	player->Delta.xz -= (float2){ x, z } / len * 0.4;
	player->Delta.y += 0.4;
	if (player->Delta.y > 0.4) { player->Delta.y = 0.4; }
}
