...
static void PopulateOre(LevelGenerator generator, BlockType ore, int a1, int a2, int a3)
{
	...
	for (int i = 0; i < ii; i++)
	{
		...
		for (int j = 0; j < jj; j++)
		{
			v1 += (float3){ sinf(r1.x) * cosf(r1.y), sinf(r1.y), cosf(r1.x) * cosf(r1.y) };
			r1.x += r2.x * 0.2;
			...
			float v2 = sinf(j * pi / jj) * a1 / 100.0 + 1.0;
...
Level LevelGeneratorGenerate(LevelGenerator generator, const char * userName, int width, int depth, int var)
{
	...
	for (int x = 0; x < w; x++)
	{
		...
		for (int y = 0; y < d; y++)
		{
			...
			if (m < 0.0) { m *= 0.8; }
			m -= powf(fminf(length2f((float2){ x / (float)w - 0.5, y / (float)d - 0.5 }) * 2.0, 1.0), 10.0) * 10.0;
			heights[x + y * w] = m;
		}
	}
